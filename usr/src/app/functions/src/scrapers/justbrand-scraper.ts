...
const scrapeProductHandlesForCategory = async (categoryUrl: string, log: (message: string) => void): Promise<string[]> => {
  ...
  while (currentPageUrl) {
    ...
    try {
      ...
      $('a.product-item__image-wrapper').each((index: number, element: cheerio.Element) => {
...
const scrapeCategories = async (log: (message: string) => void): Promise<Omit<JustBrandCategory, 'products'>[]> => {
  ...
  try {
    ...
    $('a.collection-grid-item').each((index: number, element: cheerio.Element) => {
      const url = $(element).attr('href');
      const name = $(element).find('.collection-grid-item__title').text().trim();
      if (url && name) {
        categories.push({
          name,
          slug: slugify(name),
