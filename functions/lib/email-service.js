...
exports.sendDispensaryApprovalEmail = sendDispensaryApprovalEmail;
exports.sendCrewMemberWelcomeEmail = sendCrewMemberWelcomeEmail;
 ...
 */
function createApprovalEmailTemplate(data) {
    return `
<!DOCTYPE html>
<html lang="en">
<head>
 ...
 */
async function sendDispensaryApprovalEmail(emailData) {
    try {
        // Create transporter
...
The Wellness Tree Team
      `,
        ...
        v2_1.logger.info(`Approval email sent successfully to ${emailData.ownerEmail}`, {
            messageId: info.messageId,
            dispensaryId: emailData.dispensaryId,
        });
    }
    catch (error) {
        v2_1.logger.error('Failed to send dispensary approval email:', {
            error: error.message,
            stack: error.stack,
            emailData: {
 ...
 */
function createCrewMemberWelcomeTemplate(data) {
 ...
 */
async function sendCrewMemberWelcomeEmail(emailData) {
    try {
        const transporter = nodemailer.createTransport(SMTP_CONFIG);
        const mailOptions = {
            from: {
...
The Wellness Tree Team
      `,
        ...
        const info = await transporter.sendMail(mailOptions);
        v2_1.logger.info(`Crew member welcome email sent to ${emailData.memberEmail}`, {
            messageId: info.messageId,
            role: emailData.memberRole,
        });
    }
    catch (error) {
        v2_1.logger.error('Failed to send crew member welcome email:', {
            error: error.message,
            emailData: {
...
async function verifyEmailConnection() {
    try {
        ...
        v2_1.logger.info('SMTP connection verified successfully');
        return true;
    }
    catch (error) {
        v2_1.logger.error('SMTP connection verification failed:', error.message);
        return false;
    }
}
