...
exports.chatWithAdvisor = (0, https_1.onCall)({ secrets: [openaiApiKey] }, async (request) => {
    ...
    if (!advisorSlug || !userMessage) {
        throw new https_1.HttpsError('invalid-argument', 'Missing required parameters: advisorSlug and userMessage.');
    }
    if (userMessage.trim().length === 0) {
        throw new https_1.HttpsError('invalid-argument', 'User message cannot be empty.');
    }
    ...
    try {
        ...
        if (userCredits < advisor.creditCostBase) {
            throw new https_1.HttpsError('failed-precondition', `Insufficient credits. You need at least ${advisor.creditCostBase} credits. You have ${userCredits}.`);
        }
