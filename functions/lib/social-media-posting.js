...
exports.getSocialAnalytics = exports.postToSocial = exports.refreshSocialToken = exports.disconnectSocialAccount = exports.connectSocialAccount = void 0;
 ...
 */
exports.connectSocialAccount = (0, https_1.onCall)({ cors: true }, async (request) => {
    if (!request.auth) {
    ...
    try {
        ...
        if (userContext === 'dispensary') {
            if (userData.role !== 'DispensaryOwner' && userData.dispensaryId !== entityId) {
                throw new https_1.HttpsError('permission-denied', 'You do not have permission to connect accounts for this dispensary');
            }
        }
        else {
            // For leaf users and influencers, they can only connect their own accounts
            if (request.auth.uid !== entityId) {
                throw new https_1.HttpsError('permission-denied', 'You can only connect accounts for yourself');
            }
        }
        ...
        throw new https_1.HttpsError('internal', error.message || 'Failed to connect account');
    }
});
/**
...
exports.disconnectSocialAccount = (0, https_1.onCall)({ cors: true }, async (request) => {
    ...
    try {
        ...
        throw new https_1.HttpsError('internal', error.message || 'Failed to disconnect account');
    }
});
/**
 * Refresh OAuth token (placeholder for OAuth implementation)
 */
exports.refreshSocialToken = (0, https_1.onCall)({ cors: true }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError('unauthenticated', 'User must be authenticated');
    }
 ...
 */
exports.postToSocial = (0, https_1.onCall)({ cors: true }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const { dispensaryId, platforms, message, imageUrls, videoUrl, link, hashtags, platformContent } = request.data;
    ...
    try {
        ...
        throw new https_1.HttpsError('internal', error.message || 'Failed to post');
    }
});
/**
 * Get social media post analytics (placeholder)
 */
exports.getSocialAnalytics = (0, https_1.onCall)({ cors: true }, async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError('unauthenticated', 'User must be authenticated');
    }
...
//# sourceMappingURL=social-media-posting.js.map