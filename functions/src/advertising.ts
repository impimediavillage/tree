 ...
 */
export const trackAdImpression = onCall(async (request) => {
  ...
  try {
    ...
    if (!adDoc.exists) {
      throw new HttpsError('not-found', 'Advertisement not found');
    }
 ...
 * Triggered by HTTP request from client
 */
export const trackAdClick = onCall(async (request) => {
  const { adId, destination, userId, trackingCode } = request.data;

  if (!adId || !destination) {
    throw new HttpsError('invalid-argument', 'Ad ID and destination are required');
  }
 ...
 */
export const trackAdConversion = onDocumentCreated('orders/{orderId}', async (event) => {
 ...
 */
export const aggregateDailyAdAnalytics = onSchedule('0 0 * * *', async (event) => {
 ...
 */
export const cleanupExpiredAds = onSchedule('0 1 * * *', async (event) => {
  const now = new Date();
    ...
    try {
      ...
      const updatePromises = expiredAdsSnapshot.docs.map(doc => 
        doc.ref.update({
          status: 'ended',
 ...
 */
export const activateScheduledAds = onSchedule('0 * * * *', async (event) => {
  const now = new Date();
    ...
    try {
      ...
      const updatePromises = scheduledAdsSnapshot.docs.map(doc => 
        doc.ref.update({
          status: 'active',
          updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        })
