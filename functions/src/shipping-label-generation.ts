...
interface PudoContact {
  name: string;
  mobile_number: string;
  email: string;
}

interface PudoParcel {
  parcel_description: string;
  submitted_length_cm: number;
  submitted_width_cm: number;
  submitted_height_cm: number;
...
interface CreateShipLogicLabelRequest {
  orderId: string;
  orderNumber: string;
  dispensaryId: string;
...
interface CreatePudoLabelRequest {
  orderId: string;
  orderNumber: string;
  dispensaryId: string;
...
interface LabelCreationResponse {
  success: boolean;
  shipmentId: string;
...
export const createShiplogicShipment = onCall(
  ...
  async (request: CallableRequest<CreateShipLogicLabelRequest>) => {
    logger.info('createShiplogicShipment invoked');

    // Validate authentication
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'Must be authenticated to generate shipping labels.');
    }
    ...
    if (!collectionAddress || !deliveryAddress) {
      throw new HttpsError('invalid-argument', 'Missing address information.');
    }

    if (!collectionContact || !deliveryContact) {
      throw new HttpsError('invalid-argument', 'Missing contact information.');
    }

    if (!parcels || parcels.length === 0) {
      throw new HttpsError('invalid-argument', 'At least one parcel is required.');
    }
    ...
    try {
      ...
      const response_data: LabelCreationResponse = {
        success: true,
        shipmentId: result.id,
        trackingNumber: result.tracking_reference,
...
export const createPudoShipment = onCall(
  ...
  async (request: CallableRequest<CreatePudoLabelRequest>) => {
