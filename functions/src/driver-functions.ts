...
export async function sendDriverWelcomeEmail(
  ...
  try {
    ...
    logger.info(`Driver approved: ${email}`, {
      displayName,
      temporaryPassword,
 ...
 */
export async function geocodeAddress(address: string, city: string, province: string, country: string): Promise<{ latitude: number; longitude: number } | null> {
  try {
    ...
    if (!apiKey) {
      logger.warn('GOOGLE_MAPS_API_KEY secret not configured. Set it via: firebase functions:secrets:set GOOGLE_MAPS_API_KEY');
      return null;
    }
    ...
    if (data.status === 'OK' && data.results && data.results.length > 0) {
      ...
      logger.info(`Geocoded successfully: ${location.lat}, ${location.lng}`);
      return {
        latitude: location.lat,
        longitude: location.lng
      };
    } else {
      logger.warn(`Geocoding failed: ${data.status}`, { fullAddress, error: data.error_message });
      return null;
    }
  } catch (error) {
...
export async function notifyDriversWithPriority(
  ...
  try {
    ...
    if (!privateDriversQuery.empty) {
      logger.info(`Found ${privateDriversQuery.size} private drivers for dispensary ${dispensaryId}`);
      
      // Notify private drivers
      const privateNotifications = privateDriversQuery.docs.map(async (driverDoc) => {
        const driverData = driverDoc.data();
        const distance = calculateDistance(
          driverData.homeLocation?.latitude || 0,
          driverData.homeLocation?.longitude || 0,
          pickupLocation.latitude,
          pickupLocation.longitude
        );

        return sendFCMPushNotification(driverDoc.id, {
    ...
    logger.info(`No private drivers found, notifying public drivers for delivery ${deliveryId}`);
    await notifyPublicDriversNearby(deliveryId, orderNumber, pickupLocation, 10);

  } catch (error) {
    logger.error('Error notifying drivers with priority:', error);
  }
}
...
async function notifyPublicDriversNearby(
  ...
  try {
    ...
    logger.info(`Found ${publicDriversQuery.size} public drivers to check`);
    ...
    for (const driverDoc of publicDriversQuery.docs) {
      ...
      if (!homeLocation?.latitude || !homeLocation?.longitude) continue;
    ...
    logger.info(`Notifying ${nearbyDrivers.length} nearby drivers`);

    // Notify drivers
    const notifications = nearbyDrivers.map(driver =>
      sendFCMPushNotification(driver.id, {
        title: '🚗 New Delivery Available',
    ...
    logger.error('Error notifying public drivers:', error);
  }
}
