...
const AdvisorCard: React.FC<AdvisorCardProps> = ({ title, description, icon: Icon, link, dataAiHint, delay = 0 }) => (
  <Card 
    className="shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col bg-card animate-fade-in-scale-up" 
...
const SignupBenefitCard: React.FC<SignupBenefitCardProps> = ({ title, buttonText, buttonLink, buttonIcon: ButtonIcon, benefits, delay = 0, dataAiHint }) => (
  <Card 
    className="shadow-xl hover:shadow-2xl transition-shadow duration-300 flex flex-col bg-card border-2 border-primary/30 animate-fade-in-scale-up"
    style={{ animationFillMode: 'backwards', animationDelay: `${delay}ms` }}
    data-ai-hint={dataAiHint || title.toLowerCase().replace(/\s+/g, '-')}
  >
    <CardHeader className="bg-muted/50 p-4 border-b border-primary/20">
...
export default function HolisticAiHubPage() {
  ...
  return (
    <div className="container mx-auto py-8 px-4 md:px-6 lg:px-8 space-y-12">
      ...
      <div className="p-6 bg-card rounded-lg shadow-xl animate-fade-in-scale-up" style={{ animationFillMode: 'backwards', animationDelay: '0.1s' }}>
      ...
      {currentUser && currentUser.role === 'Super Admin' && (
        <Card className="bg-secondary/10 border-secondary/50 shadow-lg animate-fade-in-scale-up" style={{ animationFillMode: 'backwards', animationDelay: '0.2s' }}>
          <CardHeader>
      ...
      {currentUser && (currentUser.role === 'User' || currentUser.role === 'LeafUser') && (
         <Card className="bg-card shadow-xl border-primary/50 animate-fade-in-scale-up" style={{ animationFillMode: 'backwards', animationDelay: '0.2s' }}>
