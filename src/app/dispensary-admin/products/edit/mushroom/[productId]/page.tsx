    ...
    <Card class="max-w-4xl mx-auto my-8 shadow-xl">
      <CardHeader>
        <div class="flex items-center justify-between">
            <CardTitle class="text-3xl flex items-center text-foreground"> <Brain class="mr-3 h-8 w-8 text-primary" /> Edit Mushroom Product </CardTitle>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} class="space-y-6">
            <div class="space-y-6">
                ...
                {watchCategory === 'Shamanic Mushrooms' && (
                  <Alert variant="destructive" class="bg-red-50 border-red-200 text-red-800">
                    <AlertTriangle class="h-5 w-5 text-red-600" />
                    <CardTitle class="text-red-700 text-lg">Legal & Responsibility Disclaimer</CardTitle>
                    <CardDescription class="text-red-600 space-y-2 mt-2">
                      ...
                      <p class="font-semibold">The Wellness Tree does not monitor your store inventory. The sole responsibility for ensuring all listed products are legal rests with you, the store owner.</p>
                    </CardDescription>
                  </Alert>
                )}
                ...
                <div class="space-y-6">
                    ...
                    {priceTierFields.map((field, index) => (
                        <div key={field.id} class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end p-3 border rounded-md relative bg-muted/30">
                            ...
                            <FormField control={form.control} name={`priceTiers.${index}.price`} render={({ field: f }) => ( <FormItem class="md:col-span-1"><FormLabel>Price ({currentUser?.dispensary?.currency}) *</FormLabel><FormControl><Input type="number" step="0.01" {...f} /></FormControl><FormMessage /></FormItem> )} />
